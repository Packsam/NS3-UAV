...
void LteSpectrumPhy::DoDispose ()
{
  ...
  m_ltePhyRxPssCallback = MakeNullCallback< void, uint16_t, Ptr<SpectrumValue> > ();//dwb
  m_ltePhyRxPSSCallback = MakeNullCallback< void, uint16_t, Ptr<SpectrumValue>,bool,bool,int > ();//dwb
  ...
  switch (m_state)
  {
    ...
    case IDLE:
    {
      ...
      */
      ...
      txParams->ctrlMsgList = ctrlMsgList;
      txParams->PSS = pss;//
      txParams->CellId = m_cellId;//测试用代
      txParams->NLos = false;//dwb
      txParams->ZeroPoint = false;//dwb
      txParams->unnecessary = 0;
      m_channel->StartTx (txParams);
          ...
          if (!m_ltePhyRxPSSCallback.IsNull ())
              {
                ...
                m_ltePhyRxPSSCallback (cellId, lteDlCtrlRxParams->psd, lteDlCtrlRxParams->NLos, lteDlCtrlRxParams->ZeroPoint, lteDlCtrlRxParams->unnecessary);//dwb
              }
        }   
