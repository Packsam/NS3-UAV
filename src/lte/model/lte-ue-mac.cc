...
LteUeMac::SendRaPreamble (bool contention)
{
  ...
  m_uePhySapProvider->SendRachPreamble (m_raPreambleId, m_raRnti);//发送RAdwb
  //std::cout<<ns3::Simulator::Now().GetMilliSeconds()<<"    发送RApreamble    "<<std::endl;
...
LteUeMac::RecvRaResponse (BuildRarListElement_s raResponse)
{
  ...
  m_cmacSapUser->NotifyRandomAccessSuccessful ();//接收RAR成功dwb
  //std::cout<<ns3::Simulator::Now().GetMilliSeconds()<<"    接收RAR    "<<std::endl;
