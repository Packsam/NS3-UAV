    ...
    case CONNECTED_HANDOVER:
      {
        ...
        m_rrcSapUser->SendRrcConnectionReconfigurationCompleted (msg);//RRC结束dwb
        //std::cout<<ns3::Simulator::Now().GetMilliSeconds()<<"    RRC完成    "<<std::endl;
       ...
       // std::cout<<m_imsi<<" "<<m_cellId<<" "<<ns3::Simulator::Now().GetSeconds()<<std::endl;
      ...
      if (params.m_componentCarrierId != 0)
      ...
      else
        {
          SaveUeMeasurements (newMeasIt->m_cellId, newMeasIt->m_rsrp,
                              newMeasIt->m_rsrq, useLayer3Filtering, newMeasIt->Nloss, newMeasIt->ZeroPoint,newMeasIt->unnecessary);//dwb
        }
    }
...
LteUeRrc::SaveUeMeasurements (uint16_t cellId, double rsrp, double rsrq,
                              bool useLayer3Filtering, bool nloss, bool zeropoint, int unnecessary)//dwb
{
      ...
      if (useLayer3Filtering)
        {
		  ...
		  if ( ((m_cellId-1)/3==(cellId-1)/3)
			  &&storedMeasIt!=m_storedMeasValues.end ()
		      &&rsrp>storedMeasIt->second.rsrp )
		  {
       ...
       storedMeasIt->second.Nloss = nloss;//dwb
       storedMeasIt->second.ZeroPoint = zeropoint;//dwb 
       storedMeasIt->second.unnecessary = unnecessary;
    }
  else
    {
      ...
      v.rsrq = rsrq;
      v.Nloss = nloss;//dwb
      v.ZeroPoint = zeropoint;//dwb
      v.unnecessary = unnecessary;
...
LteUeRrc::SendMeasurementReport (uint8_t measId)
{
  ...
  measResults.Nloss = servingMeasIt->second.Nloss;//dwb
  measResults.ZeroPoint = servingMeasIt->second.ZeroPoint;//dwb
  measResults.unnecessary = servingMeasIt->second.unnecessary;
