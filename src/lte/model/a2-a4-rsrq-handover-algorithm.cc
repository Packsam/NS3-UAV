                                           ...
                                           LteRrcSap::MeasResults measResults)
{
  ...
  if (measResults.measId == m_a2MeasId)
    {
     // std::cout<<(int)measResults.rsrqResult<<std::endl;//测试用
      NS_ASSERT_MSG (measResults.rsrqResult <= m_servingCellThreshold,
                     "Invalid UE measurement report");
      EvaluateHandover (rnti, measResults.rsrqResult, measResults.rsrpResult, measResults.Nloss, measResults.ZeroPoint,measResults.unnecessary);
    }
...
A2A4RsrqHandoverAlgorithm::EvaluateHandover (uint16_t rnti,
                                             uint8_t servingCellRsrq, uint8_t servingCellRsrp, bool nloss, bool zeropoint,int unnecessary)
{
  ...
  if (it1 == m_neighbourCellMeasures.end ())
  ...
  else
    {
      ...
      //uint16_t secondNeighbourCellId = 0;//测试用变量
      ...
      for (it2 = it1->second.begin (); it2 != it1->second.end (); ++it2)
        {
         ...
         /* else if((it2->second->m_rsrq  > secondNeighbourRsrp)//测试用代码
               && IsValidNeighbour (it2->first))
                    {
                      secondNeighbourCellId = it2->first;
                      secondNeighbourRsrp = it2->second->m_rsrq; 
                    }*/

        }
      //std::cout<<bestNeighbourCellId<<" "<<(int)bestNeighbourRsrq<<" and "<<secondNeighbourCellId<<" "<<(int)secondNeighbourRsrp<<" and serving RSRQ is"<<(int)servingCellRsrq<<ns3::Simulator::Now()<<std::endl;//测试用代码
      // Trigger Handover, if needed
      if (bestNeighbourCellId > 0)
        {
          NS_LOG_LOGIC ("Best neighbour cellId " << bestNeighbourCellId);
          //dynamic threshold change by dwb 
          if((int)servingCellRsrq <=50){
            m_neighbourCellOffset = 0;
          }else{
            m_neighbourCellOffset = (int)round((int)servingCellRsrq/30);
          }
          ...
          if ((bestNeighbourRsrq - servingCellRsrq) >= m_neighbourCellOffset)
            {
             ...
             m_handoverManagementSapUser->TriggerHandover (rnti,
                                                            bestNeighbourCellId);
              num0fH0++;
              std::cout<<ns3::Simulator::Now().GetSeconds()<<"   "<<num0fH0<<"   "<<(int)servingCellRsrp<<"   "<<(int)servingCellRsrq<<"   "<<nloss<<"   "<<zeropoint<<std::endl;
              }
            }
        }
