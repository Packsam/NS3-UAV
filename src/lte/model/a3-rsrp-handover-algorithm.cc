              ...
              if (it->haveRsrpResult)
                {
                  ...
                  else if((it->rsrpResult > secondNeighbourRsrp)//测试用代码
                      && IsValidNeighbour (it->physCellId))
                    {
                     // secondNeighbourCellId = it->physCellId;
                      //secondNeighbourRsrp = it->rsrpResult; 
                    }
                }
              else
                {
...
//std::cout<<bestNeighbourCellId<<" "<<(int)bestNeighbourRsrp<<" and "<<secondNeighbourCellId<<" "<<(int)secondNeighbourRsrp<<" and serving RSRQ is"<<(int)measResults.rsrpResult<<ns3::Simulator::Now()<<std::endl;//测试用代码
          if (bestNeighbourCellId > 0)
            {
              ...
              m_handoverManagementSapUser->TriggerHandover (rnti,
                                                            bestNeighbourCellId);
              num0fH0++;
              std::cout<<num0fH0<<" "<<ns3::Simulator::Now().GetSeconds()<<std::endl;
            }
        }
      else
        {
