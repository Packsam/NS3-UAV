  ...
  for (it = m_ueMeasurementsMap.begin (); it != m_ueMeasurementsMap.end (); it++)
    {
      ...
      newEl.Nloss = (*it).second.Nloss;//dwb
      newEl.ZeroPoint = (*it).second.ZeroPoint;//dwb
      newEl.unnecessary = (*it).second.unnecessary;
              ...
              for (std::list<RarLteControlMessage::Rar>::const_iterator it = rarMsg->RarListBegin (); it != rarMsg->RarListEnd (); ++it)
                {
                  if (it->rapId != m_raPreambleId)
                  ...
                  else
                    {
                      ...
                      QueueSubChannelsForTransmission (ulRb);
                      // pass the info to the MAC
                      m_uePhySapUser->ReceiveLteControlMessage (msg);//收到RAR消息
...
void
LteUePhy::ReceivePSS (uint16_t cellId, Ptr<SpectrumValue> p, bool Nloss, bool ZeroPoint, int unnecessary)
{
  ...
  static int second;
  static int third;
  static double firstRsrp;
  static double secondRsrp;
  static double thirdRsrp;
  if (cellId == 1)
  {
    first = 1;
    second = 1;
  ...
  if (rsrp_dBm >firstRsrp)
  {
    first = cellId;
    firstRsrp = rsrp_dBm;
  }
  else if(rsrp_dBm > secondRsrp)
  {
    second = cellId;
    secondRsrp = rsrp_dBm;
  }
  else if(rsrp_dBm > thirdRsrp)
  {
    third = cellId;
    thirdRsrp = rsrp_dBm;
  }
  static double time;
  if ((cellId ==57)&&((ns3::Simulator::Now().GetSeconds()-time)>0.1)&&(firstRsrp-secondRsrp<5))
  {
    std::cout<<first<<" "<<second<<"  "<<third<<"      "<<ns3::Simulator::Now().GetSeconds()<<std::endl;
  ...
  if (itMeasMap == m_ueMeasurementsMap.end ())
  ...
  else
    {
      ...
      (*itMeasMap).second.Nloss = Nloss;
      (*itMeasMap).second.ZeroPoint = ZeroPoint;
      (*itMeasMap).second.unnecessary = unnecessary;
    }
