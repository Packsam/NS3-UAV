...
int
MobilityModel::In2DRangeOf(Ptr<const MobilityModel> mobility) 
{
  Vector2D position = Vector2D(DoGetPosition().x,DoGetPosition().y);
  Vector2D oposition = Vector2D(mobility->DoGetPosition ().x,mobility->DoGetPosition ().y);
  Vector2D nextoposition = Vector2D((mobility->DoGetPosition ().x+0.5*mobility->DoGetVelocity().x),(mobility->DoGetPosition ().y+0.5*mobility->DoGetVelocity().y));
  if(CalculateDistance(position,nextoposition)<CalculateDistance(position,oposition))
  {
    return 1;
  }
  return -1;
