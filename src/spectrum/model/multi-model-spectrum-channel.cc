                  ...
                  if (rxParams->txAntenna != 0)
                    {
                      ...
                      rxParams->ZeroPoint = rxParams->txAntenna->ZeroPoint;//dwb
                      NS_LOG_LOGIC ("txAntennaGain = " << txAntennaGain << " dB");
                      pathLossDb -= txAntennaGain;
                      //------------------------------------------------//测试用代码
                      bool isPSS = txParams->PSS;
                      if(isPSS)//根据路径判断是否需要切换dwb
                      {
                       if(rxParams->ZeroPoint)
                       {
                         Vector move (0.1*(receiverMobility->GetVelocity().x),0.1*(receiverMobility->GetVelocity().y),0.1*(receiverMobility->GetVelocity().z));
                         Vector nextReceiver = receiverMobility->GetPosition ()+move;
                         Angles nextAngles (nextReceiver, txMobility->GetPosition ());
                         double nextAntennaGain = rxParams->txAntenna->GetGainDb (nextAngles);
                         if(nextAntennaGain == -25)
                         {
                            rxParams->unnecessary = 2;
                         }
                       }else
                       {
                          rxParams->unnecessary = txMobility->In2DRangeOf(receiverMobility);
                       }
                      }
                      //---------------------------------------------------
                    }
                  ...
                  if (m_propagationLoss)
                    {
                      ...
                      rxParams->NLos = m_propagationLoss->Nlos;
                    }                    
